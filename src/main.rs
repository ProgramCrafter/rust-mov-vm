...
struct Memory {
    ...
    io: (std::io::Bytes<termion::AsyncReader>, RawTerminal<std::io::Stdout>)
}
impl Memory {
    ...
    fn load_opcode(&self, address: usize) -> (u16, u16) {
        ...
        ((i32 >> 16).try_into().unwrap(), (i32 & 0xFFFF).try_into().unwrap())
    }
...
struct Registers {
    buffer: [i64; 36],
    triggers: HashMap<usize,
               (Vec<fn(usize, &mut [i64; 36], &mut Memory)>,
                Vec<fn(usize, &mut [i64; 36], &mut Memory)>)>
}
impl Registers {
    fn get_triggers_pair(&mut self, index: usize) ->
      &mut(Vec<fn(usize, &mut [i64; 36], &mut Memory)>,
           Vec<fn(usize, &mut [i64; 36], &mut Memory)>) {
        self.triggers.entry(index).or_insert((Vec::new(), Vec::new()))
    }
    ...
    fn init_triggers(&mut self) {
        ...
        fn cio_trig(trig: usize, buffer: &mut [i64; 36], memory: &mut Memory) {
            if trig == 1 {
                ...
                print!("{}", char::from_u32(buffer[16].try_into().unwrap()).unwrap());
            } else {
        ...
        fn io_trig(trig: usize, buffer: &mut [i64; 36], _memory: &mut Memory) {
            if trig == 1 {
                print!("{}", char::from_u32(buffer[18].try_into().unwrap()).unwrap())
            } else {
                buffer[19] = 10;
            }
        }
        ...
        fn atz_trig(_trig: usize, buffer: &mut [i64; 36], _memory: &mut Memory) {
            buffer[23] = if buffer[20] == 0 {buffer[21]} else {buffer[22]};
        }
        ...
        fn mem_trig(trig: usize, buffer: &mut [i64; 36], memory: &mut Memory) {
            if trig == 1 {
                memory.store64(buffer[26].try_into().unwrap(), buffer[24].try_into().unwrap());
            } else {
                buffer[24] = memory.load64(buffer[26].try_into().unwrap()).try_into().unwrap();
            }
        }
...
fn main() {
    ...
    let mut regs: Registers = Registers {buffer: [0; 36], triggers: HashMap::new()};
    let mut mem : Memory = Memory {buffer: box_array![0; MEMORY_SIZE], io: (stdin, stdout)};
    ...
    let mut i: u64 = 0;
    print!(" ... stopped: tick={}", loop {
        let addr = regs.buffer[27] as usize;
        
        // println!("addr={} sub0={}", addr, regs.buffer[3]);
        
        if addr >= MEMORY_SIZE {break i;}
        // if i > 10000 {break i;}
        
        let (src, dst) = mem.load_opcode(addr);
        // println!("{}>{}", src, dst);
        
        let val = if src & 0x8000 != 0 {(src & 0x7FFF) as i64}
                                  else {regs.get(src.try_into().unwrap(), &mut mem)};
